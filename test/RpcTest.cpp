...
namespace
{
                   ...
                   const std::string& privateKeyPemFile)
    {
        ...
        Casper::Secp256k1Key secp256k1Key(privateKeyPemFile);
...
void PutDeploy_Transfer_Test(void) {
  using namespace Casper;
  sendCoins(pub_key, pub_key_target, amount_str, privKeyPemFile);
  sendCoins(pub_key_target, pub_key, amount_str, privKeyPemFileTarget);
}
